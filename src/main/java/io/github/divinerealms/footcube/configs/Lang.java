...
@Getter
public enum Lang {
  PLUGIN_STRING("plugin-string", "&7[&eFC&7]"),
  ADMIN_STRING("admin-string", "&7[&cAdmin&7]"),

  NO_PERM("no-perm", "{plugin-string}&cNemate dozvolu (&4{0}&c) za komandu &6/&e{1}&c!"),
  UNKNOWN_COMMAND("unknown-command", "{plugin-string}&cNepoznata komanda."),
  USAGE("usage", "{plugin-string}Usage: /{0}"),
  PLAYER_NOT_FOUND("player-not-found", "&cPlayer not found."),
  ON("toggle.on", "&aON"),
  OFF("toggle.off", "&cOFF"),

  RELOAD("reload", "{admin-string}FootCube reloaded!"),
  FC_TOGGLE("toggle.status", "{admin-string}FootCube Matches turned {0}"),
  ADMIN_STATSET("statset", "{admin-string}Updated {0} for player {1} to {2}"),

  HITDEBUG_CHARGED("debug.hits.charged", "{plugin-string}&dCharged &8| {0} &8| {1}KP &8| &e{2}PW&7, &d{3}CH"),
  HITDEBUG_REGULAR("debug.hits.regular", "{plugin-string}&aRegular &8| {0} &8| &a{1}KP"),

  OR("or", "&e|&b"),
  SPY_ENABLED("spy.enabled", "{plugin-string}&aSpy mode enabled"),
  SPY_DISABLED("spy.disabled", "{plugin-string}&aSpy mode disabled"),
  ...
  JOIN_BLOCKED("join.blocked", "{plugin-string}&cYou cannot join matches for another {0}min {1}s due to leaving while losing."),

  TEAM_USAGE("team.usage", String.join(System.lineSeparator(), "&b/fc team [3v3{0}4v4] [player]", "&b/fc team accept/decline/cancel")),
  TEAM_CANCEL_OTHER("team.cancel.other", "{plugin-string}&a{0} cancelled his request to team with you"),
  TEAM_CANCEL("team.cancel.self", "{plugin-string}&aYou successfully cancelled your team request"),
  ...
  TEAM_ACCEPT_OTHER("team.accept.other", String.join(System.lineSeparator(), "{plugin-string}&aYou succesfully teamed with {0}", "&aYou must wait for there to be place for a team, this won't take long")),
  TEAM_ACCEPT_SELF("team.accept.self", String.join(System.lineSeparator(), "{plugin-string}&aYou succesfully teamed with {0}", "&aYou must wait for there to be place for a team, this won't take long")),
  TEAM_SUCCESS_1("team.success-1", "{plugin-string}&aYou successfully teamed with {0}"),
  TEAM_SWITCH("team.switch", "{plugin-string}&aYou switched teams so that {0} and {1} could team"),
  TEAM_DECLINE_OTHER("team.decline.other", "{plugin-string}&a{0} declined your team request"),
  TEAM_DECLINE_SELF("team.decline.self", "{plugin-string}&aYou successfully declined the team request"),
  TEAM_DECLINE_JOIN("team.decline.join", "{plugin-string}&aYou declined the team request because you joined a match"),
  ...
  TEAM_WANTS_TO_TEAM_OTHER("team.wants-to-team.other", String.join(System.lineSeparator(), "{plugin-string}&a{0} wants to team with you on a {1}v{1} match", "&b/fc team accept &aor &b/fc team decline &ato answer the team request")),
  TEAM_WANTS_TO_TEAM_SELF("team.wants-to-team.self", String.join(System.lineSeparator(), "{plugin-string}&aYou succesfully sent {0} a team request for a {1}v{1} match", "&b/fc team cancel &ato cancel this")),
  ...
  TEAM_CANCELLED_OTHER("team.cancelled-other", "{plugin-string}&a{0} cancelled his team request"),
  TEAM_CANCELLED_JOIN("team.cancelled-join", "{plugin-string}&aYou cancelled your team request because you joined a match"),
  ...
  STATS("stats.info", String.join(System.lineSeparator(),
      "&e---------------------------------------------",
      "&r {plugin-string}&6{0} statistika:",
      "&r &r",
  ...
  DENIED_COMMAND("denied-command", "{plugin-string}&cNe mozete koristiti tu komandu"),
  ...
  LEFT("leave.left", "{plugin-string}&aYou left the match."),
  LEAVE_LOSING("leave.losing", "{plugin-string}&cYou left while losing! You were fined $200 and cannot join for 30 minutes."),

  UNDO("undo", "{plugin-string}&aUndo successful"),

  CLEARSTORE_SUCCESS("clear-store.success", "{plugin-string}&aYou succesfully wiped {0}'s store"),
  ...
  SETUP_ARENA_START("setup-arena.start", String.join(System.lineSeparator(), "{plugin-string}&aYou just started to setup an arena", "&aIf you got here by accident, do &b/fc undo", "&aStep 1: Stand in the middle block behind the line of the blue goal and look at the red goal, then do &b/fc set")),
  SETUP_ARENA_ALREADY_SOMEONE("setup-arena.already-someone", "&c{0} is already setting up an arena"),
  SETUP_ARENA_FIRST_SET("setup-arena.first-set", String.join(System.lineSeparator(), "{plugin-string}&aFirst location successfully set.", "&aNow do the same for the red goal")),
  SETUP_ARENA_SUCCESS("setup-arena.success", "{plugin-string}&aYou successfully setup the arena"),

  CLEAR_ARENAS_SUCCESS("clear-arenas.success", "{plugin-string}&aYou successfully wiped all arenas"),
  ALREADY_ENOUGH_CUBES("already-enough-cubes", "&cThere already are enough cubes"),

  BALANCE("balance", "{plugin-string}&aYou currently have #{0}"),

  TEAMMATE_LEFT("team.teammate-left", "{plugin-string}&aYour teammate left, the teaming got cancelled"),

  GLOBAL_CHATTING("global-chatting", "{plugin-string}&aUse &b/g [message] &afor global chatting"),

  TAKEPLACE_ANNOUNCEMENT("takeplace.announcement", "{plugin-string}&6ANNOUNCEMENT: &aA player left a {0}v{0} match during discussion phase"),
  TAKEPLACE_ANNOUNCEMENT_2("takeplace.announcement-2", "{plugin-string}&6ANNOUNCEMENT: &aA player left a {0}v{0} match, he was running {1} with {2} seconds to play"),
  ...
  SPEED_USAGE("speed.usage", "{plugin-string}&aYou got speed {0} for {1} seconds, you can use this again in {2} seconds"),

  WELCOME("match.welcome", "{plugin-string}&aWelcome to the match, you are on the {0} team"),
  STARTING("match.starting", String.join(System.lineSeparator(), "{plugin-string}&aThere are enough players to start, the match will start in 30 seconds. You now have time to discuss about your strategy.", "&2TIP: &aChoose someone to be goalkeeper.", "&aUse &b/tc [Message] &afor teamchat.")),
  ...
  MATCH_STARTED("match.started", "{plugin-string}&aThe match has started, good luck"),
  MATCH_PROCEED("match.proceed", "{plugin-string}&aThe match will now proceed"),
  MATCH_TIMES_UP("match.times-up", "{plugin-string}&aTime's up! The {0} team has won"),
  MATCH_WIN_CREDITS("match.win-credits", "{plugin-string}&aYou got 15 credits for winning"),
  MATCH_WINSTREAK_CREDITS("match.winstreak-credits", "{plugin-string}&6&lYou get 100 credits bonus for winning {0} times in a row!!!"),
  MATCH_TIED("match.tied", "{plugin-string}&aTime's up! The game is tied"),
  MATCH_TIED_CREDITS("match.tied-credits", "{plugin-string}&aYou got 5 credits for ending tied"),
  MATCH_SCORE_CREDITS("match.score-credits", "{plugin-string}&aYou got 10 credits for scoring"),
  MATCH_SCORE_HATTRICK("match.score-hattrick", "{plugin-string}&6&lYou get 100 credits bonus for making a hat-trick"),
  MATCH_GOAL("match.goal", "{plugin-string}&6&lGOAL!!! &a{0} scored a goal for the {1} team"),
  ...
  MATCH_SCORE_OWN_GOAL("match.score-own-goal", "{plugin-string}You scored an own goal! $200 has been deducted."),
  MATCH_SCORE_OWN_GOAL_ANNOUNCE("match.score-own-goal-announce", "{plugin-string}&6&lOWN GOAL! &a{0} scored a goal for the opposing team"),
  MATCH_OWN_GOAL_LEAVE("match.own-goal-leave", "{plugin-string}&c{0} &cje izbačen iz igre zbog auto golova!"),

  BEST_UPDATING("best.updating", String.join(System.lineSeparator(), "{plugin-string}&aScores are updating, please wait", "&6Updating 500 players per second with {0} players to go")),
  BEST_HEADER("best.header", String.join(System.lineSeparator(), "{plugin-string}&6All FootCube highscores:", "&bBest ratings:")),
  ...
  HELP("help", String.join(System.lineSeparator(), "{plugin-string}&6List of commands with /fc", "&b/fc join [3v3{0}4v4]", "&b/fc team [3v3{0}4v4] [player]", "&b/fc team accept/decline/cancel", "/fc group", "/fc takeplace", "/fc stats", "/fc best")),
  HELP_ADMIN("help-admin", String.join(System.lineSeparator(), "{admin-string}&b/fc setuparena [3v3{0}4v4]", "{admin-string}&b/fc cleararenas")),

  SPEED_ITEM_NAME("speed-item.name", "Speed boost"),
  SPEED_ITEM_LORE("speed-item.lore", "Right click to get speed"),

  SCOREBOARD_TITLE("scoreboard.title", "Match"),
  SCOREBOARD_TIMELEFT("scoreboard.time-left", "Time left:"),
  SCOREBOARD_RED_TEAM("scoreboard.red-team", "Red team:"),
  SCOREBOARD_BLUE_TEAM("scoreboard.blue-team", "Blue team:"),
  ...
  PRACTICE_AREA_SET("practice-area-set", "{plugin-string}&fUspešno postavljena lokacija &b{0}&f (&o{1}, {2}, {3}&f)."),

  FC_DISABLED("fc-disabled", "{plugin-string}&cUlaženje u mečeve je privremeno deaktivirano od strane admina."),
  ...
  MATCHES_LIST_HEADER("matches.list.header", String.join(System.lineSeparator(),
      "&e---------------------------------------------",
      "&r {plugin-string}&eLista aktivnih mečeva:",
      "&r &r")),
